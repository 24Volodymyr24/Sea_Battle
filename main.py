...
def on_closing():
    global app_running
    if messagebox.askokcancel("Quit the game", "Do you want to quit the game?"):
        app_running = False
        tk.destroy()


tk.protocol("WM_DELETE_WINDOW", on_closing)
tk.title("Sea Battle Game")
tk.resizable(0, 0)
...
def mark_igrok(igrok_mark_1):
    if igrok_mark_1:
        ...
        t3.configure(text="Move of Player #2"+add_to_label)
        t3.place(x=size_canvas_x + menu_x // 2 - t3.winfo_reqwidth() // 2, y=size_canvas_y)
    else:
        t1.configure(bg="red")
        t0.configure(bg="#f0f0f0")
        ...
        t1.configure(text="Player #2" + add_to_label)
        t1.place(x=size_canvas_x + menu_x + size_canvas_x // 2 - t1.winfo_reqwidth() // 2, y=size_canvas_y + 3)
        t3.configure(text="Move of Player #1")
        t3.place(x=size_canvas_x + menu_x // 2 - t3.winfo_reqwidth() // 2, y=size_canvas_y)
...
b0 = Button(tk, text="Show the ships \n Player #1", command=button_show_enemy1)
b0.place(x=size_canvas_x + menu_x // 2 - b0.winfo_reqwidth() // 2, y=10)

b1 = Button(tk, text="Show the ships \n Player #2", command=button_show_enemy2)
b1.place(x=size_canvas_x + menu_x // 2 - b1.winfo_reqwidth() // 2, y=60)

b2 = Button(tk, text="Re-start!", command=button_begin_again)
...
def hod_computer():
    ...
    if check_winner2():
        winner = "Player #2 wins"+add_to_label
        winner_add = "(All enemy ships of Player 1 are shot down)!!!!!"
    ...
    if ip_x >= s_x + delta_menu_x and ip_x <= s_x + s_x + delta_menu_x and ip_y < s_y and not hod_igrovomu_polu_1:
        ...
        if points2[ip_y][ip_x - s_x - delta_menu_x] == -1:
            ...
            if check_winner2_igrok_2():
                ...
                winner_add = "(All enemy ships of Player #2 are shot down)!!!!!"
                print(winner, winner_add)
                points1 = [[10 for i in range(s_x)] for i in range(s_y)]
                points2 = [[10 for i in range(s_x)] for i in range(s_y)]
